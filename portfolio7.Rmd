...
contact_long <- interventions %>%
  select(contactChildren, contactAdults, contactChildAdult) %>%
...
d_temporal <- dWide %>%
  select(book, temporalSetting)

knowledge_vars <- c("contempPrej", "historicalPrej", "knowRace", 
                    "knowCulture", "antiRacism", "structuralRacism", "policeViolence")

d_strategies <- interventions %>%
  select(book, all_of(knowledge_vars))
...
knowledge_by_temporal <- intertemp %>%
  select(temporalSetting, all_of(knowledge_vars)) %>%
