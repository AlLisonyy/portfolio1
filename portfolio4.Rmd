...
result <- cocor.dep.groups.overlap(r.jk = r_jk, r.jh = r_jh, r.kh = r_kh, n = n)
result@ hotelling1940
# See the results
print(result)
...
# Function to run cocor and return results
compare_corrs <- function(var_name, Thesis_scale) {
  r_jk <- cor(Thesis_scale$BFS_score, Thesis_scale[[var_name]], use = "pairwise.complete.obs")
  r_jh <- cor(Thesis_scale$BPS_score, Thesis_scale[[var_name]], use = "pairwise.complete.obs")
  r_kh <- cor(Thesis_scale$BFS_score, Thesis_scale$BPS_score, use = "pairwise.complete.obs")
  n <- nrow(Thesis_scale)
  result <- cocor.dep.groups.overlap(r.jk = r_jk, r.jh = r_jh, r.kh = r_kh, n = n)
  ...
  tibble(
    ...
    conclusion = ifelse(!is.na(p_Hotelling) && p_Hotelling < 0.05, "Different", "Not Different"),
    hotelingt = result@ hotelling1940$statistic
    )
  }

# Run for all 28 variables
results_df <- map_dfr(target_vars, compare_corrs, Thesis_scale = Thesis_scale)
