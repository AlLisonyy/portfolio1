...
partial_BFSpih <- Thesis_scale %>% 
  ...
  drop_na()
pcor.test(partial_BFSpih$BFSp_score, partial_BFSpih$ihs_score, partial_BFSpih$LIE_score)
...
partial_BSthoughtnarcgrand <- Thesis_scale %>% 
  select(BSthought, narcgrand_score, LIE_score) %>% 
  drop_na()
pcor.test(partial_BSthoughtnarcgrand$BSthought, partial_BSthoughtnarcgrand$narcgrand_score, partial_BSthoughtnarcgrand$LIE_score)

##4.2 BSthought and Hypersensitive Narcissism Scale
partial_BSthoughtnarchyper <- Thesis_scale %>% 
  select(BSthought, narchyper_score, LIE_score) %>% 
  drop_na()
pcor.test(partial_BSthoughtnarchyper$BSthought, partial_BSthoughtnarchyper$narchyper_score, partial_BSthoughtnarchyper$LIE_score)
...
# 6. BFSe vs BSthought
results_BFSp_BPS <- map_dfr(target_vars, compare_correlates, x_name = "BFSe_score", y_name = "BSthought", Thesis_scale = Thesis_scale)

# 7. BFSp vs BSthought
results_BFSp_BPS <- map_dfr(target_vars, compare_correlates, x_name = "BFSp_score", y_name = "BSthought", Thesis_scale = Thesis_scale)
...
```{r}
compare_partial_correlates <- function(var_name, x_name, y_name, Thesis_scale) {
  # Calculate partial correlations, controlling for LIE_score
  partial_corr1 <- tryCatch(pcor.test(Thesis_scale[[x_name]], Thesis_scale[[var_name]], Thesis_scale[["LIE_score"]], method = "pearson"), error = function(e) NULL)
  partial_corr2 <- tryCatch(pcor.test(Thesis_scale[[y_name]], Thesis_scale[[var_name]], Thesis_scale[["LIE_score"]], method = "pearson"), error = function(e) NULL)
  partial_corr3 <- tryCatch(pcor.test(Thesis_scale[[x_name]], Thesis_scale[[y_name]], Thesis_scale[["LIE_score"]], method = "pearson"), error = function(e) NULL)
  
  # Check if partial correlations were calculated successfully
  if (is.null(partial_corr1) || is.null(partial_corr2) || is.null(partial_corr3)) {
    return(tibble(
      variable = var_name,
      r_pred1 = NA,
      r_pred2 = NA,
      ...
      p_Hotelling = NA,
      p_Steiger = NA,
      conclusion = "Error during partial correlation calculation",
      hotelingt = NA
    ))
  }
  
  # Extract partial correlation estimates
  r_jk <- as.numeric(partial_corr1$estimate)
  r_jh <- as.numeric(partial_corr2$estimate)
  r_kh <- as.numeric(partial_corr3$estimate)
  n <- nrow(Thesis_scale)
  
  # Check if correlations are valid numbers between -1 and 1
  if (!is.na(r_jk) && !is.na(r_jh) && !is.na(r_kh) && 
      all(c(r_jk, r_jh, r_kh) >= -1 & c(r_jk, r_jh, r_kh) <= 1)) {
    
    result <- tryCatch(cocor.dep.groups.overlap(r.jk = r_jk, r.jh = r_jh, r.kh = r_kh, n = n), 
    ...
    if (!is.null(result)) {
      ...
      return(tibble(
        ...
        p_Hotelling = NA,
        p_Steiger = NA,
        conclusion = "Error during cocor calculation",
        hotelingt = NA
      ))
    }
  } else {
    return(tibble(
      ...
      p_Hotelling = NA,
      p_Steiger = NA,
      conclusion = "Invalid correlation values",
      hotelingt = NA
    ))
  }
}
...
## Variable List
target_vars <- c("BFSe_score", "BFSp_score", "BPS_score", "sdtnarc_score", "sdtmach_score", 
                 "sdtpsych_score", "narcgrand_score", "narchyper_score", "mach_score", "psycho_score", 
                 "psychopri_score", "psychosec_score", "se_score", "hon_score", "emo_score", "extra_score", 
                 "agree_score", "cons_score", "open_score", "ihs_score", "nclos_score", "ncog_score", 
                 "openTS_score", "closeTS_score", "intuitiveTS_score", "effortTS_score", "activeopen_score", 
                 "fi_score", "crt_score", "word_score", "ns_score", "BSthought")
...
results_BFS_BPS_partial <- map_dfr(target_vars, compare_partial_correlates, x_name = "BFS_score", y_name = "BPS_score", Thesis_scale = Thesis_scale)
